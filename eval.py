...
class MultiWozEvaluator(object):
        ...
        for dial_id in dials:
            ...
            for turn_num, turn in enumerate(dial):
                ...
                if cfg.same_eval_act_f1_as_hdsa:
                    ...
                    '''
                    if '<sos_a>' not in turn['aspn_gen']:
                        pred_acts = self.reader.aspan_to_act_list('<sos_a> '+turn['aspn_gen']+' <eos_a>')
                        true_acts = self.reader.aspan_to_act_list('<sos_a> '+turn['aspn']+' <eos_a>')
                    else:
                    '''
                    aspn_gen=turn['aspn_gen'].strip('<sos_a>').strip('<eos_a>')
                    aspn=turn['aspn'].strip('<sos_a>').strip('<eos_a>')
                    pred_acts = self.reader.aspan_to_act_list(aspn_gen)
                    true_acts = self.reader.aspan_to_act_list(aspn)
                    tp, fp, fn = _get_tp_fp_fn(true_acts, pred_acts)
                '''
                if fp + fn !=0:
    ...
    def context_to_response_eval(self, data, eval_dial_list = None, same_eval_as_cambridge=False):
        ...
        for dial_id in dials:
            ...
            if cfg.col_samples:
                if match>0:
                    match_sample[dial_id]=dial[1:]
                else:
                    mismatch_sample[dial_id]=dial[1:]
                if success>0:
                    success_sample[dial_id]=dial[1:]
                else:
                    unsuccess_sample[dial_id]=dial[1:]
        ...
        if cfg.col_samples:
            match_path=os.path.join(cfg.eval_load_path,'mismatch_dials.json')
            success_path=os.path.join(cfg.eval_load_path,'unsuccess_dials.json')
    ...
    def _evaluateGeneratedDialogue(self, dialog, goal, real_requestables, counts,
        ...
        for t, turn in enumerate(dialog):
            ...
            sent_t=unidecode.unidecode(sent_t)
            # sent_t = turn['resp']
            for domain in goal.keys():
                ...
                if same_eval_as_cambridge:
                        ...
                        if domain not in dom_pred:  # fail
                            continue
                #只有value_name或value_id才能表征实体，也就是判断是否match，其他的placeholder都用来判断success
