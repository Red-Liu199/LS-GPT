...
# This script is the main script. We can run our supervised 
# and semi-supervised (including Semi-VL and Semi-ST)
# experiments through this script
...
class Modal(object):
    ...
    def pretrain(self,posterior=False):
        ...
        epoch_th=0.1*cfg.epoch_num if cfg.gpt_path=='distilgpt2' else -1
    ...
    def semi_VL(self):
        ...
        for epoch in tqdm(range(epoch_num)):
            ...
            if epoch==0:
                ...
                logging.info('time of querying database:{:.3f} min'.format(self.db_time/60))
    ...
    def validate_fast(self,data='dev'):
        ...
        result_path=os.path.join(cfg.eval_load_path,'result1.csv') if cfg.test_data_path=='' \
        ...
        self.reader.save_result('w', results, field, write_title='DECODED RESULTS:',result_name='result1.csv')
    ...
    def validate_act(self,data='dev'):
        result_path=os.path.join(cfg.eval_load_path,'result1.csv')
    ...
    def validate_pos(self,data='dev'):
        ...
        logging.info('inference time:{:.3f} min'.format((time.time()-st)/60))
        self.reader.save_result('w', results, field, write_title='DECODED RESULTS:',result_name='result.csv')
        eval_result={}
        _, _, eval_result['act_F1']=self.evaluator.aspn_eval(results)
        eval_result['joint_acc'], eval_result['db_acc'] = compute_jacc(results,return_db=True)
        logging.info('Joint acc:{:.3f}, Act_F1:{:.3f}, DB_acc:{:.3f}'.\
