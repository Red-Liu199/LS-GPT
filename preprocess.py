...
class DataPreprocessor(object):
    ...
    def delex_by_annotation(self, dial_turn):
        ...
        for s in span:
            ...
            try:
                #修改记录：增加了下方的条件判断
                #源代码中将value_price和value_pricerange混合在一起了，但是应该不影响最终的评估结果，
                # 因为value_price并不参与inform和success的计算，其作用只是单纯的去词汇化
                if s[2] in ['expensive','moderate','cheap']:
                    u[s[3]]='[value_pricerange]'
                else:
                    u[s[3]] = '[value_'+slot+']'
            except:
                u[5] = '[value_'+slot+']'
    ...
    def delex_by_valdict(self, text):
        ...
        text = re.sub(r'\$\d+|\$?\d+.?(\d+)?\s(pounds?|gbps?)', '[value_price]', text)#有具体数字才替换为value_price
